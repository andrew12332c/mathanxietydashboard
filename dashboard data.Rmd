...
# Define the absolute path to your anonymized data
data_path_anon <- "C:/Users/andre/OneDrive/Documents/acelab dashboard/mathanxietydashboard/data/combined_anonymized.csv"
data_path_orig <- "C:/Users/andre/OneDrive/Documents/acelab dashboard/mathanxietydashboard/data/combined_output.csv"
...
rum_means$bar_val <- (rum_means$mean_rum - 4) * 1.5  # center at 0: scale -3 (negative) to +3 (positive)
if (nrow(rum_means) > 0) {
  ggplot(rum_means, aes(x = condition, y = bar_val, fill = condition)) +
    ...
    scale_y_continuous(breaks = seq(-3, 3, 1), limits = c(-3.5, 3.5),
                      name = "Rumination scale (−3 = negative thoughts ← → +3 = positive thoughts)") +
    labs(x = "", title = "B1. Post-Event Rumination") +
    theme_minimal() + theme(legend.position = "none")
}
```
